...
const slotIsActive = (slot, now = new Date()) => {
  const time = now.toTimeString().slice(0, 5)
  return slot.day === now.toLocaleDateString('en-US', { weekday: 'long' }) && time >= slot.time.slice(0, 5) && time <= (slot.end_time?.slice(0, 5) || addHour(slot.time))
}

function Auth({ onLogin }) {
  const [registering, setRegistering] = useState(false); const [teacherRegistration, setTeacherRegistration] = useState(false); const [verifying, setVerifying] = useState(false)
  const [form, setForm] = useState({ username: '', email: '', password: '', otp: '' }); const [error, setError] = useState('')
  async function submit(event) { event.preventDefault(); setError(''); try { if (verifying) { await api.verifyRegistration({ username: form.username, otp: form.otp }); setVerifying(false); setRegistering(false); setTeacherRegistration(false); setError('Email verified. Sign in now.'); return } if (registering) { if (teacherRegistration) await api.registerTeacher(form); else await api.register(form); setVerifying(true); return } const tokens = await api.login(form.username, form.password); saveTokens(tokens); onLogin() } catch (err) { setError(err.message) } }
  const beginRegistration = (asTeacher) => { setRegistering(true); setTeacherRegistration(asTeacher); setError('') }
  return <main className="auth"><div className="auth-card"><span className="eyebrow">DAILY RHYTHM</span><h1>{verifying ? 'Verify your email' : registering ? `Create your ${teacherRegistration ? 'teacher' : 'student'} account` : 'Welcome back'}</h1><p>{verifying ? `Enter the code sent to ${form.email}.` : 'Keep every class, streak, and goal in view.'}</p><form onSubmit={submit}>{verifying ? <label>Six-digit code<input required inputMode="numeric" pattern="[0-9]{6}" maxLength="6" value={form.otp} onChange={e => setForm({ ...form, otp: e.target.value })} /></label> : <><label>Username<input required value={form.username} onChange={e => setForm({ ...form, username: e.target.value })} /></label>{registering && <label>Email<input required type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} /></label>}<label>Password<input required type="password" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} /></label></>}{error && <div className="error">{error}</div>}<button className="primary">{verifying ? 'Verify email' : registering ? 'Send verification code' : 'Sign in'}</button></form>{!verifying && <>{<button className="link-button" onClick={() => registering ? (setRegistering(false), setError('')) : beginRegistration(false)}>{registering ? 'Back to sign in' : 'New here? Register as a student'}</button>}{!registering && <button className="teacher-link" onClick={() => beginRegistration(true)}>Register as a teacher</button>}</>}</div></main>
}

function App() {
  const [authenticated, setAuthenticated] = useState(Boolean(getTokens())); const [account, setAccount] = useState(null); const [view, setView] = useState('Dashboard'); const [error, setError] = useState('')
